const { Board, Group } = require("../models/board-models");
...
async function createGroupInBoard(req, res) {
  try {
    ...
    // const createdGroup = await Board.findByIdAndUpdate({_id: req.params.id, $set: { groups: req.body.groups.title}});
    // res.status(201).json(createdGroup);
  } catch (error) {
    res.status(500).json({
...
async function getGroupsInBoard(req, res) { 
    ...
    try{
        ...
        res.status(500).json({
            message: 'something went wrong retrieving groups from board, try again later!'
        })
    }
}
...
async function getGroupInBoard(req, res) { 
    ...
    try { 
        ...
        res.status(500).json({
            message: 'something went wrong retrieving group from board, try again later!'
        })
    }
}
...
async function archiveGroupInBoard(req, res) { 
    ...
    try{
        ...
        res.status(500).json({
            message: 'something went wrong archiving group in board, try again later!'
        })
    }
